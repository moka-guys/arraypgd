with open('252846911385_S01_Guys121919_CGH_1100_Jul11_2_1_1.txt', encoding='utf-8') as fefile:
    import csv
    ...
    for row in readfeline:
        ...
        if line < 1:
            print(row)
            with open('filtered.txt', mode='w', encoding='utf-8') as filteredfile:
        ...
        if line > 0 and line < 10:
            print(row)
            with open('filtered.txt', mode='a', encoding='utf-8') as filteredfile:
                filteredfilewriter = csv.writer(filteredfile, delimiter='\t')
                filteredfilewriter.writerow(row)                
